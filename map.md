...
- **StrainMap**: a BioCyc-style single-line "beads on a string" gene map filling the whole
  rectangle. Genes are rectangles on one line, arrow direction = strand, colored by the
  presence call of the selected query (present = green, partial = amber, absent = red,
  n/a = gray). The old blue identity-alignment rows are removed. Gene labels (symbol,
  fallback locus_tag) are drawn inside the boxes when zoomed in enough. Variant markers
  (SNP = red tick, deletion = purple tick, insertion = blue tick) are drawn on the gene
  when the visible window is smaller than ~20 kb. Positions shown as a ruler only.
